...
def obtaincorrxt(file_j, path): 
    ...
    Sp_a = np.reshape(Sp_aj, (steps,L,3)) #[0:steps:fine_res]; we want the original 641 (formerly 961) steps, not 26 or fewer
    print('Sp_a.shape: ' , Sp_a.shape)
    stepcount = min(steps, 4001) 
    # 512 steps; but if jump 10/20 is taken, 520 is the last step
    Sp_a = Sp_a[:stepcount] 
    ...
    for ti in range(mconsv.shape[0]):
        for x in range(mconsv.shape[1]):
            mconsv[ti,x] = (Sp_a[ti,x])/(alpha)**x #0.5* : factor of 1/2 only when averaging is done over sites x, x+1
