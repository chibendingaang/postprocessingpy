...
if Mu == 1 and Lambda ==0:
    param = 'qwdrvn'; last_site = 600; first_site = 100 	#another choice could be 100-400
if Mu == 0 and Lambda ==1:
    param = 'qwhsbg'; last_site = 600; first_site = 100
...
def plot_arrivaltime():
    ...
    if param=='qwhsbg': prefac = 0.8; v_butf = 1.6
    if param=='qwdrvn': prefac = 0.5; v_butf = 1.3
    #approximate values of the butterfly velocities we know
    xlist = np.linspace(first_site,last_site,6, dtype=int) #100 for qwhsbg #np.array([100,200,300,400,500,600,700]) \
    ...
    ax1.set_xlabel('x')
    ax1.set_ylabel(r'$t_{D_0}$')
    #ax1.set_title(r'Arrival times of decorrelator front $\lambda = {}, \mu = {} $ '.format(Lambda, Mu))
   
    #ax2 = plt.subplot(1,2,2)
    ax2 = ax1.inset_axes([0.1, 0.625, 0.28, 0.28]) #takes up 7.84% of the area of the main plot
    ...
    for xi,xp in enumerate(xlist[0:-1]):
        ...
        ax2.plot(bincenters, y, '-', label=str(xp))
        ax2.set_xlim((-22,22))
        ax2.set_xlabel(r'$t_{D_0} - \langle t_{D_0} \rangle$')
        ax2.legend()
        ...
        gausffit = prefac*gaussf(xfit, 0, np.sqrt(centraltavg[-200])/(xsites[-200]**(1/3.)))

        #if xp== 50:  plt.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--o',  label=str(xp))
        if xi== 0: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--^',  label=str(xp))
        if xi== 1: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--8',  label=str(xp))
        if xi== 2: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--s',  label=str(xp))
        if xi== 3: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--v',  label=str(xp))
        if xi== 4: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--*',  label=str(xp))
        if xi== 5: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--o',  label=str(xp))
        if xi== 6: ax3.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--8',  label=str(xp))
        #if xp== 600: plt.plot(bincenters/(xp**(pfit)), xp**(pfit)*y, '--8',  label=str(xp))
        ax3.plot(xfit, gausffit, '--k', linewidth=3)
        #plt.xlabel(r'$(t_{D} - \langle t_{D} \rangle)/x^{b}$')
        #plt.ylabel(r'$x^{b} PDF$')
        ax3.set_xlim(-2,2)
        ax3.set_xlabel(r'$(t_{D_0} - \langle t_{D_0} \rangle)/x^{1/3}$')
        ax3.set_ylabel(r"$x^{1/3}$ p.d.f")
        #ax3.set_title(r'Distribution collapse for the x^{}.PDF vs (t - $\langle t_D0 \rangle $)/x^{}'.\
        #format(round(pfit,2),round(pfit,2)));
    
    plt.savefig('./plots/avg_arrivaltime_vs_x_{}_Lambda_{}_Mu{}_{}_Dth_{}_{}bins_{}config.png'.format(L,Lambda,Mu,thresh,threshfactor,nbins,np.shape(timeregister)[0]))
