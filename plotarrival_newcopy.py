...
if Mu == 1 and Lambda ==0:
    param = 'qwdrvn'; last_site = 450; first_site = 200 	#another choice could be 100-400
...
def plot_arrivaltime():
    ...
    if param=='qwdrvn': prefac = 0.5; v_butf = 1.35
    ...
    ax1.legend(handles=handle1, title=r'$x = $', frameon=True, ncol=2, loc='lower center')
    ax2.set_xlim((-18,18))
    ax2.set_ylabel(r'p.d.f.', fontsize=20)
    ax2.set_xlabel(r'$\mathit{t_{D_0} - \langle t_{D_0} \rangle}$',fontsize=20)
    ax2.tick_params(labelsize=16)
    ax3.tick_params(labelsize=16)
    ax3.set_xlim(-2,2)
    ax3.set_xlabel(r'$\mathit{(t_{D_0} - \langle t_{D_0} \rangle)/x^{1/3}}$',fontsize=20)
    ax3.set_ylabel(r"$\mathit{x^{1/3}} . $ p.d.f.",fontsize=20)
        
    plt.savefig('./plots/avg_arrivaltime_vs_x_{}_{}_dt{}_{}_Dth{}_{}bins_{}config.png'.format(L,param,dtstr, thresh,threshfactor,nbins,np.shape(timeregister)[0]))
