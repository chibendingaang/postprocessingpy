...
fig2, axes = plt.subplots(nrows = 4, ncols = 2, figsize = (10.8,14.4))
...
for qi, qval in enumerate(qmode_arr):
    ...
    axes[qi,0].plot(T, Sp3q[:,0], label=rf'$S^3, q= 0 $'); #axes[qi,0].legend() 
    axes[qi,0].plot(T, np.abs(Spq_sq[:,0]), label=rf'$|S(q,t)|^2, q= 0 $'); #axes[qi,0].legend() 
    ...
    axes[qi,0].legend()
    axes[qi,1].legend()
    axes[qi,0].set_xlabel('t'); axes[qi,0].set_ylabel(r'$Re[S^{(i)}(t)]$')
...
plt.tight_layout()
#plt.savefig(f'./plots/Spinq_abs_vs_t_modltd_q{qthread}.pdf')
plt.savefig(f'./plots/Spinq_abs_vs_t_pertbd_q{qthread}.pdf')
