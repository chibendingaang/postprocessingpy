...
steps = int(0.25*dtsymb* (Sp_aj.size/(3*L)- 1)) #0.25 works but 0.1 doesn't, go figure why
print('steps = ', steps)
...
def obtainspinsnew(steps,file_j): 
    ...
    Sp_a = np.reshape(Sp_aj, (2*steps+1,L,3))[0:2*step+1:10];
    ...
    if param == 'qwhsbg':
        ...
        for ti,t in enumerate(range(0,2*step+1,10)):
            print(ti)
            for x in range(L):
                Cxt[ti,x] = np.sum(np.sum((Sp_a*np.roll(np.roll(Sp_a,-x,axis=1),-ti,axis=0))[:-ti],axis=2)) 	#multiplying an array Sxt[t=ti] with a scalar Sxt[t=0,L=0]
        #array[-:ti] acts like step function: includes matrix products uptil that T-ti
        return Cxt/(L*(2*step//10 +1)), energ_1 
    ...
    if param == 'qwdrvn':
        ...
        return Cxt/(L*(2*step//10+1)), energ_eta1
...
for conf in range(begin, end):
    if param == 'qwhsbg':
        Cxt, energ_1  = obtainspinsnew(steps,conf)
    if param == 'qwdrvn':
        Cnnxt, energ_eta1 = obtainspinsnew(steps,conf)
    ...
    if param == 'qwhsbg':
        f = open(f'./{Cxtpath}/Cxt_t_{dtstr}_{epstr[epss]}_{conf}to{conf+1}config.npy','wb'); np.save(f, Cxt); f.close()
    if param == 'qwdrvn':
        f = open(f'./{Cxtpath}/Cnnxt_t_{dtstr}_{epstr[epss]}_{conf}to{conf+1}config.npy','wb'); np.save(f, Cnnxt); f.close()
    
    '''
    if param == 'qwhsbg':
        f = open(f'./{energypath}/H_tot_t_{dtstr}_{epstr[epss]}_{conf}to{conf+1}config.npy','wb'); np.save(f, energ_1); f.close()
    if param == 'qwdrvn':
        f = open(f'./{energypath}/H_tot_etat_{dtstr}_{epstr[epss]}_{conf}to{conf+1}config.npy','wb'); np.save(f, energ_eta1); f.close()
