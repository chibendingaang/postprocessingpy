...
if epss ==3: 
    filepath = f'./Dxt_storage/emin3_{param}'
    filerepo = f'{filepath}/out2048_{dtstr}.txt'
if epss ==4: 
    filepath = f'./Dxt_storage/L1024'
    filerepo = f'{filepath}/allnpysamples.txt'

f = open(filerepo)
print('filerepo name: ', filerepo)
kksmall = np.array([x for x in f.read().splitlines()])[begin:end] #([x for x in f.readlines()]) #filename
f.close()
print('# of files in the filerepo : ', kksmall.shape)
print('first and last elements : ', kksmall[0], kksmall[-1])


if epss == 3: filesteps = np.load(f'{filepath}/{filesize}_{dtstr}/{kksmall[-1]}', allow_pickle=True).shape[0]
else: filesteps = np.load(f'{filepath}/{kksmall[-1]}', allow_pickle=True).shape[0]
steps = min(filesteps, steps)
print('# steps in the array file: ', filesteps)
Dxtavg = np.zeros((2*steps+1,L)) #2*steps+1
#if epss==4: Dxtavg = np.zeros((steps+1, L))

for k in kksmall :
    if epss==3: Dxtk = np.load(f'{filepath}/{filesize}_{dtstr}/{k}', allow_pickle=True);
    else: Dxtk = np.load(f'{filepath}/{k}', allow_pickle=True);
    Dxtavg += Dxtk[:2*steps+1]

Dxtavg = Dxtavg/len(kksmall)
D_th = 100*epsilon**2
#Dxtavg is the concatenated symmetric decorrxt array first calculated in calc_decorrfly
...
VB = 0
for t in range(101,min(steps,501)):
    v = t_fact*check_x_intercept(0.5*t_fact*logDxt[t-1]/t)/t
    # above is just v = dx/dt; dx is obtained by calling check_x_intercept func
    VB += v
    print(' v_i = ', v)
    #print('t, logdxtbyepssq : ', t, 2.5*logDxt[t-1,0]/t)
VB = VB/len(range(101,min(501,steps)))

print('vB = ', VB)

#print('t, logDxt : ', 1, 2.5*logDxt[0]/t)
tinit = int(sys.argv[8]) #11-25; 20 is most likely the origin point of the light-cone
...
ax2.tick_params(axis='both', which='both', direction='in', width=1.2,labelsize=16)
...
first_legend = axes.legend(handles=handle1,frameon=False, title=r'$t = $', ncol=2, loc='upper right')
...
ax2.set_xlabel(r'$\mathit{x} $',fontsize=20)
ax2.set_ylabel(r'$\mathit{D(x,t)} $',fontsize=20)
