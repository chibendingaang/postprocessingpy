...
if choice ==2:
    ...
    if Lambda == 1 and Mu == 1: param = 'qwa2b0'
    path = f'./{param}/{dtstr}/temp'	#path2 # the param is drvn or hsbg here; choice = 2

#for all relevant purposes:
#epss = 3; choice = 2; fine_res = t_smoothness = 1
...
def calc_autoCxt(autoCxt_, steps, spin):
    for ti,t in enumerate(range(0,steps,fine_res)):
        print('time: ', t)
        for x in range(L):
            autoCxt_[ti,x] = np.sum(spin[0,:,:]*np.roll(spin,-x,axis=1),axis=2) #,axis=2))  	#multiplying an array Sxt[t=ti] with a scalar Sxt[t=0,L=0]
    return autoCxt_
...
def obtaincorrxt(file_j, path): 
    ...
    '''
    #Cxt = calc_Cxt(Cxt,stepcount, Sp_a); 
    Cnnxt = calc_Cxt(Cnnxt,stepcount, Sp_ngva)
    #CExt = calc_CExt(CExt,stepcount, E_loc); 
    #CENxt = calc_CExt(CENxt,stepcount, Eeta_loc)
    #return Cxt[1:]/L, Cnnxt[1:]/L, CExt[1:]/L, CENxt[1:]/L, energ_1, energ_eta1
    return Cnnxt[1:]/L, energ_eta1
    #CENxt[1:]/L,energ_1
...
Cxtpath = 'Cxt_series_storage/L{}_responder'.format(L)
...
for conf in range(begin, end):
    print(conf)
    Cnnxt, energ_eta1  = obtaincorrxt(conf, path)
    #if param == 'qwdrvn':
    ...
    f = open(f'./{Cxtpath}/Cnnxt_t_{dtstr}_jump{fine_res}_{epstr[epss]}_{param}_{conf}to{conf+1}config.npy','wb'); np.save(f, Cnnxt); f.close()
