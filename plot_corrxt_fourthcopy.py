...
def opencorr(L,path):
    ...
    '''
    #why 32400/8000/202 filenumber
    #Sp_aj = np.loadtxt('{}/spin_a_{}.dat'.format(path,str(begin+301))) 
    #demofile to open and check array length: steps, L, 3
    # the demofile only works for files storedin Cxt_storage: epss = 4, choice = 1.
    # the range of files for epss = 3, choice = 0 is 202 - 600; 1001-1064, ..., 1401 - 1432, 1501-1532
    '''
    print('~resolution~ smoothness between steps = ', fine_res, '\n #steps = ', steps) # , '\n stepcount = min(steps, 525)' )  
    steps = min(steps, 5121) #pick fewer steps where necessary
    ...
    for k, fk  in enumerate(filename):
        Cnnxtk = np.load(f'./{Cxtpath1}/{fk}', allow_pickle=True)[:steps//fine_res]
...
def plot_corrxt(magg):
    ...
    ax2 = axes.inset_axes([0.1, 0.525, 0.25, 0.25])
    ax3 = axes.inset_axes([0.675,0.525,0.25,0.25])
    ...
    Cnnxt_, filenam2 =  opencorr(L, path)
    ...
    fig.tight_layout()
    plt.savefig('./plots/{}_vs_x_L{}_{}_{}_{}config.png'.format(corr[magg],L, param,epstr[epss],len(filename[magg])))
    #plt.show()
