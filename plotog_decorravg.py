...
end = int(sys.argv[5]) #rows at which to stop parsing the splitdecorrhsbg.txt file
interval = 1
...
#for k in kk:
#	if k > 250:
#		Dxtk = np.load('./Dxt_storage/Dxt_{}_lamda_{}_mu_{}_sample_{}to{}.npy'.format(L, Lambda, Mu, k,k+1))
	#elif k > 1001 and k<11000:
	#	Dxtk = np.load('Dxt_{}_lamda_{}_mu_{}_sample_{}to{}.npy'.format(L, Lambda, Mu, k,k+100))
	#elif k>11000:
	#	Dxtk = np.load('Dxt_{}_lamda_{}_mu_{}_sample_{}to{}.npy'.format(L, Lambda, Mu, k,k+100))
#	Dxtavg += Dxtk
...
for k in kksmall :
	Dxtk = np.load('./Dxt_storage/{}'.format(k), allow_pickle=True);
	#print(Dxtk); 
	Dxtavg += Dxtk
