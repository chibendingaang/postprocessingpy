...
if Mu == 0 and Lambda ==1:
    param = 'qwhsbg'; last_site = 700; first_site = 100
if Mu ==1 and Lambda ==1:
    param = 'qwa2b0'; last_site = 700; first_site = 100
...
def plot_arrivaltime():
    ...
    site0_traverse = [-90,-60,-30,0,30,60,90] 

    for i,si in enumerate(site0_traverse):
        startsite = first_site + si
        ...
        plt.subplot(1,2,1)
        if i==0: plt.plot(xsites, np.sqrt(centraltavg_), 'o',linewidth=0.5, label='sigma_t_D');
        plt.plot(xsites, p_opt[0]*np.power(xsites , p_opt[1]) , '--', label = str(p_opt[0])+'(x-{})^'.format(startsite) +str(p_opt[1])) #+'+'+str(p_opt[-1]))    
        plt.subplot(1,2,2)
        if i==0: plt.loglog(xsites, np.sqrt(centraltavg_), 'o',linewidth=0.5, label='sigma_t_D');
        plt.loglog(xsites, p_opt[0]*np.power(xsites , p_opt[1]) , '--', label = str(p_opt[0])+'(x-{})^'.format(startsite)+str(p_opt[1])) #+'+'+str(p_opt[-1]))    
    ...
    plt.subplot(1,2,1)
    plt.plot(xsites, par_opt[0]*np.power(xsites , 1/3.) , '--', label = str(par_opt[0])+'x^'+str(1/3)) #xsites[0] ideally   
    plt.subplot(1,2,2)
    plt.loglog(xsites, par_opt[0]*np.power(xsites , 1/3.) , '--', label = str(par_opt[0])+'x^'+str(1/3)) #xsites[0] ideally   
    plt.legend()
    plt.title(r'$\sigma_{t_D}$ vs x for different initial sites; Dth/$\epsilon^2 = $' + '{} '.format(threshfactor))
    plt.savefig('./plots/avg_arrivaltime_logstdev_{}_Lambda_{}_Mu{}_{}_Dth_{}_{}config.png'.format(L,Lambda,Mu,epstr, threshfactor, len(filename)))
    ...
    plt.grid()
    plt.xlim(0,last_site); plt.ylim(0,int(last_site/1.3))
    plt.xlabel('x')
    plt.ylabel(r'$t$')
