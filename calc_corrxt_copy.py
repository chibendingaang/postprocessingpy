...
if Lambda == 1 and Mu == 0: 
    if choice==0: param = 'xphsbg'
    if choice==1: param = 'qwhsbg'
    if choice==2: param = 'hsbg'
if Lambda == 0 and Mu == 1: 
    if choice ==0: param = param = 'xpdrvn'
    if choice ==1: param = 'qwdrvn'
    if choice ==2: param = 'drvn'
if Lambda == 1 and Mu == 1: 
    if choice ==0:param = 'xpa2b0'
    if choice ==1:param = 'qwa2b0'
...
#dictionary names
epstr = dict()
epstr[3] = 'eps_min3'; epstr[4] = 'eps_min4'; epstr[6] = 'eps_min6'; epstr[8] = 'eps_min8' 

start  = time.perf_counter()

path0 = f'./{param}/L{L}/2emin3' #usually xpdrvn
path1 = f'./L{L}/{epstr[epss]}/{param}' 
#the largest file data is currently here: 17042023
#usually qwdrvn #the epss = 3 for this path
path2 = f'./{param}/{dtstr}'	
# the param is drvn or hsbg here; choice = 2
...
def calc_Cxt(Cxt_, steps, spin,ti):
    print('time: ', ti)
    for x in range(L):
...
def calc_CExt(CExt_, steps, elocal, ti):
    for x in range(L):
...
def obtaincorrxt(file_j, path): 
    ...
    ''' 
    #delegate the following code to two separate fucntions:
    ...
    for ti,t in enumerate(range(0,steps,fine_res)):
        Cxt[ti] = calc_Cxt(Cxt,stepcount, Sp_a,ti); 
        Cnnxt[ti] = calc_Cxt(Cnnxt,stepcount, Sp_ngva,ti)
        CExt[ti] = calc_CExt(CExt,stepcount, E_loc,ti); 
        CENxt[ti] = calc_CExt(CENxt,stepcount, Eeta_loc,ti)
...
for conf in range(begin, end):
    print(conf)
    Cxt, Cnnxt, CExt, CENxt, energ_1, energ_eta1  = obtaincorrxt(conf, path1)
    #if param == 'qwdrvn':
