...
#filepath
path = f'./xpdrvn/L{L}/2emin3'
fine_r = 2 #int(sys.argv[1])
#is fine_r = 2 compatible to all input spin files?
...
def openfile(m):
    #m = qmode_arr[0]
    inp_file = f"{path}/spin_a_pertbd{m}0001.dat" #modulated or pertbd 
...
def calc_dxt(sp_a, sp_b):
    # spin configuration sp_a and its perturbed copy sp_b
...
def calc_avgmag(m,steps):
    ...
    """
    num_slices = 1
    #averaging over slices of periodicity num_slices is equivalent to taking fourier transform with that wavenumber
    
    autocorr_m = np.zeros(steps)
    sp_a = openfile(m); print('m = ', m)
    print('spinarr shape: ', sp_a.shape)

    #sp_a = np.reshape(spina, (steps , L, dim3)) #[0:steps:fine_r] 
    avgmag = np.sum(sp_a[:L//num_slices], axis=1)/L
    ...
    print('avgmag shape: ', avgmag.shape)
...
#f1, f2, f3, auc1, auc2, auc3 = check_auc(2)
...
def plot_mkt():
    ...
    for k in qmode_arr[0:]: #range(0,m+1):
        ...
        #f1, f2,  auc1, auc2 = check_auc(k, inputf)
        f1, f2,  auc1, auc2 = check_auc(k, inputg)
        ...
        ax1.plot(np.arange(steps), autocorr_m, label= f'k = {k}') #fine_r * np.arange(T)
        ...
        ax2.plot(np.arange(steps)[500:540], autocorr_m[500:540])
        ax2.set_xlabel('t')
